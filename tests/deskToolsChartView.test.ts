...
vi.mock("../services/analysis/KlineService", async (importOriginal) => {
  const actual =
    await importOriginal<typeof import("../services/analysis/KlineService")>();
  return { ...actual, fetchKlines: vi.fn() };
});
vi.mock("../services/analysis/MarketDataService", async (importOriginal) => {
  const actual =
    await importOriginal<
      typeof import("../services/analysis/MarketDataService")
    >();
  return {
    ...actual,
...
vi.mock(
  "../services/analysis/HybridIntelligenceService",
  async (importOriginal) => {
    const actual =
      await importOriginal<
        typeof import("../services/analysis/HybridIntelligenceService")
      >();
    return {
      ...actual,
...
describe("desk tools — chart awareness", () => {
  ...
  it("get_chart_view reports candles, mark price, levels and the symbol/interval", async () => {
    ...
    const result = await executeDeskTool(
      {
        id: "c1",
        name: "get_chart_view",
        arguments: { symbol: "BTCUSDT", interval: "4h" },
      },
      {
        chartLevels: [
          { label: "Entry", price: 101.5 },
          { label: "Stop", price: 98 },
        ],
      },
    );
    ...
    expect(result.content).toContain(
      "CHART VIEW — BTCUSDT · 4h · last 2 candles",
    );
    expect(result.content).toContain("Live mark price: 104.7");
    expect(result.content).toContain(
      "Levels drawn on the chart: Entry 101.5 · Stop 98",
    );
  ...
  it("get_chart_view prefers the canvas websocket mark over REST premiumIndex", async () => {
    vi.mocked(fetchKlines).mockResolvedValue(candles as never);
    vi.mocked(fetchMarkIndex).mockResolvedValue({
      markPrice: 100,
      indexPrice: 99.9,
      lastFundingRate: 0.0001,
      nextFundingTime: Date.now() + 3_600_000,
    ...
    expect(result.content).toContain("Live mark price: 105");
    expect(result.content).not.toContain("Live mark price: 100");
  });

  it("get_chart_view ignores an invalid REST mark when the canvas mark is available", async () => {
    vi.mocked(fetchKlines).mockResolvedValue(candles as never);
    vi.mocked(fetchMarkIndex).mockResolvedValue({
      markPrice: 0,
      indexPrice: 99.9,
  ...
  it("get_chart_view NEVER lowercases the monthly interval (1M ≠ 1m)", async () => {
    ...
    expect(vi.mocked(fetchKlines).mock.calls.some((c) => c[1] === "1m")).toBe(
      false,
    );
  });
  ...
  it("both tools are declared with model-facing schemas", async () => {
    const { DESK_TOOL_DEFINITIONS } =
      await import("../services/analysis/DeskToolsService");
  ...
  it("get_chart_view translates the USER DRAWINGS into shape lines", async () => {
    ...
    expect(result.content).toContain(
      "DRAWINGS ON THE CHART (user + model — 2 shapes)",
    );
