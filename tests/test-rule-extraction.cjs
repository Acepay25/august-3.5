...
const InvalidationRuleService_1 = require("./InvalidationRuleService");
...
const mockTrade = {
    ...
    outcome: types_1.TradeOutcome.LOSS,
    analysis: {
        coinName: 'BTCUSDT',
        direction: 'Long',
        confidence: 'High',
...
async function testExtraction() {
    ...
    try {
        ...
        const newRules = await (0, InvalidationRuleService_1.addRulesFromPostMortem)(postMortemText, mockTrade, types_1.AIProvider.GEMINI);
        ...
        if (hasRegexRule)
            console.log('✅ Regex Logic: PASS');
        else
            console.error('❌ Regex Logic: FAIL (Expected 200 EMA rule)');
        if (hasLLMRule)
            console.log('✅ LLM Logic: PASS (or Regex caught it too)');
        else
            console.warn('⚠️ LLM Logic: MAYBE FAIL (Volume rule not found, check LLM response)');
    }
    catch (e) {
