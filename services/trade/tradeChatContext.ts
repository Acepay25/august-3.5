...
export const TRADE_CHAT_SYSTEM_PROMPT = [
    ...
    'KEY LEVELS: when your answer identifies the important levels on this chart, END it with a fenced block tagged key-levels — one per line as `LABEL | PRICE | short context` (e.g. "R1 | 112000 | swing high / first test above"; "VWAP | 111400 | intraday pivot"; label it S/R/VWAP so the card colors it). The app turns that block into a Key Levels card the user can toggle onto the chart (pin, preview, show-all), so PUBLISH your levels this way instead of only naming them in prose. Use exact numbers from the packet/tools — never invent one.',
