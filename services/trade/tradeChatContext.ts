...
export const buildTradeChatContext = ({ symbol, interval, packetMarkdown, fetchedAtMs, drawingsDescription, onScreenDescription, plansDescription, liveMarkPrice, formingCandle }: TradeChatContextInput): string => {
    ...
    return [
        ...
        'You can also GROW with this desk: write_memory_note saves a durable lesson straight into the trader notebook, get_notebook_map shows what you already know before you write, propose_skill / revise_skill / amend_memory / forge_tool queue improvements (skills, memory corrections, new tools) that a human then approves — propose them when this session taught you something that will matter on the next chart. When the user asks you to LEARN THIS CHART, scan_chart_skills reads the entire candle history (regimes, swings, gaps, every book detector with historical win-rates) and drafts skills grounded in what actually worked — it is slow, so call it at most once per turn. And you can DRAW on this chart like the user does: mark_trade_levels lays an Entry/Stop/TP plan as labeled lines, draw_on_chart adds a trendline/ray/zone/fib/text, get_all_timeframes pulls every timeframe plus the book at once. The drawings block stamps when each shape was drawn and how far the mark has moved since — honour a "POSSIBLY STALE" flag by re-checking the level before you lean on it.',
    ].filter(Boolean).join('\n\n');
};
...
export const TRADE_CHAT_SYSTEM_PROMPT = [
    ...
    'KEY LEVELS (optional): when the levels themselves ARE the point of your answer, you MAY end it with a fenced block tagged key-levels — one per line as `LABEL | PRICE | short context` (e.g. "R1 | 112000 | swing high / first test above"; "VWAP | 111400 | intraday pivot"; label it S/R/VWAP so the card colors it). The app turns that block into a Key Levels card the user can toggle onto the chart (pin, preview, show-all). This is a publishing tool, not a ritual: answer in prose when naming levels inline reads better, and never force a block just to have one. If you do publish one, use exact numbers from the packet/tools — never invent one.',
