...
// API-key enabled free models from OpenRouter
...
export const summarizeChartImage = async (image: File, chartNumber: number, modelName: string): Promise<{ uiSummary: string; fullSummary: string }> => {
    try {
        ...
        **INSTRUCTIONS:**
        ...
        console.error("Error in OpenRouter summarizeChartImage:", error);
        return {
...
` : `**CONTEXTUAL DATA:**
    ...
    // Note: Most OpenRouter free models don't support vision, so we skip image embedding
    ...
    try {
        ...
        console.error("OpenRouter analysis JSON parsing failed:", error, "Response:", responseText);
...
export const searchStrategies = async (query: string, activeFrameworks: string[], modelName: string): Promise<StrategySearchResult[]> => {
    ...
    try {
        ...
        console.error("Failed to parse OpenRouter strategy search results:", e);
        return [];
    }
};
...
export const discoverStrategies = async (chatHistory: Message[], activeFrameworks: string[], modelName: string): Promise<StrategySearchResult[]> => {
    ...
    try {
        ...
        console.error("Failed to parse OpenRouter strategy discovery results:", e);
        return [];
    }
};
...
Generate the updated Global Memory JSON object.
    ...
    try {
        ...
        console.error("OpenRouter updateGlobalMemory JSON parse failed:", responseText);
