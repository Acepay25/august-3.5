...
const getClient = (): OpenAI => {
    // Groq New API key from environment (inlined at build time via vite.config.ts)
    const GROQ_API_KEY = process.env.GROQ_NEW_API_KEY;
    if (!GROQ_API_KEY) throw new Error("GROQ_NEW_API_KEY is not set in environment");
...
` : `**CONTEXTUAL DATA:**
    ...
    const effectiveUserPrompt = `${formattedPrompt}${truncatedImages}\n\n${truncatedPattern}\n\n${truncatedInsights}\n\n${truncatedMemory}\n\nOUTPUT VALID JSON ONLY.`;
    console.log(`[Groq New TPM Opt] Input truncated - Images: ${truncatedImages.length}, Pattern: ${truncatedPattern.length}, Insights: ${truncatedInsights.length}, Memory: ${truncatedMemory.length}`);
    ...
    const completion = await groq.chat.completions.create({
        ...
        max_tokens: getMaxTokens(modelName, 4096), // Ensure complete outputs, limited for GPT-OSS 120B
    });
