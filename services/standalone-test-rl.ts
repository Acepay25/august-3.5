...
const getPreferenceObject = async <T>(key: string): Promise<T | null> => {
    const val = localStorage.getItem(key);
    return val ? JSON.parse(val) : null;
};

const setPreferenceObject = async <T>(key: string, value: T): Promise<void> => {
    localStorage.setItem(key, JSON.stringify(value));
};

// --- COPIED SERVICE CODE (With slight adaptations for mock) ---
...
const ReinforcementSignalService = {
    ...
    async recordSignal(
        ...
        try {
            ...
            if (store.signals.length > 1000) store.signals = store.signals.slice(-1000);
