...
export const StatCard: React.FC<{
    ...
    <div className="bg-zinc-800 rounded-xl border border-white/5 p-3 sm:p-4">
        <h4 className="text-ui-xs sm:text-xs font-bold text-zinc-500 uppercase tracking-wider mb-2 sm:mb-3">{title}</h4>
        {items.length > 0 ? (
                ...
                {items.map((item, i) => (
                    <div key={i} className="flex items-center justify-between">
                        ...
                        <div className="text-right">
                            ...
                            {item.subtext && <span className="text-ui-xs text-zinc-500 ml-1">{item.subtext}</span>}
                        </div>
                    </div>
                ))}
            </div>
