...
/**
 * JobsPane: the "status stack" of background work — every
 ...
 *
 * Extracted from JobsDrawer so the same body can mount inside the
 * sidebar's TERMINAL tab (inline pane) and the slide-out drawer.
 *
 * Data sources: the JobQueue snapshot (queued/running work) + each skill's
 * latest automated-eval verdict (completed audit trail). Re-reads while open.
 */
...
const JobsPane: React.FC = () => {
    ...
    return (
        <div className="min-h-0 flex-1 overflow-y-auto custom-scrollbar px-4 py-3" data-testid="jobs-pane">
            {!hasContent && (
