...
const AnalystPanel: React.FC<{
  ...
  const [typedText, isFinished] = useTypingEffect(text, 4); // Faster base speed for smoother UI
      ...
      <div className="flex-1 p-4 sm:p-5 overflow-y-auto custom-scrollbar relative">
        {showLoadingState ? (
          <div className="flex flex-col items-center justify-center h-full text-zinc-500 space-y-3 animate-pulse">
            ...
            <span className="text-xs font-mono uppercase tracking-widest">{text === null ? 'Initializing Neural Net...' : 'Decoding Output...'}</span>
          </div>
        ) : (
...
const LiveAnalysisView: React.FC<LiveAnalysisViewProps> = ({ isVisible, onClose, thoughts, geminiModelName, deepseekModelName, zhipuModelName, groqModelName, groqNewModelName, groqAlt2ModelName, openrouterModelName, onAllTypingComplete }) => {
  ...
  const activeAnalysts = useMemo(() => {
    // Standard mode: show all enabled providers
    const active: string[] = [];
    ...
    <div className="fixed inset-0 bg-zinc-950/95 z-50 flex items-center justify-center p-4 sm:p-8 backdrop-blur-sm animate-fade-in" style={{ transition: 'opacity 0.2s ease-in-out' }}>
      <div className="flex flex-col w-full h-full max-w-7xl mx-auto">
        <header className="flex items-center justify-between mb-4 sm:mb-6 flex-shrink-0">
          <div>
            ...
            <p className="text-zinc-500 text-xs sm:text-sm mt-1 font-medium">
              Multiple LLMs are analyzing market structures in real-time.
            </p>
          </div>
        ...
        <main className={`flex-1 grid ${gridCols} gap-4 sm:gap-6 min-h-0`}>
          {activeAnalysts.includes('gemini') && geminiModelName && (
            <AnalystPanel
              title="Gemini"
          ...
          {activeAnalysts.includes('deepseek') && deepseekModelName && (
            <AnalystPanel
              title="DeepSeek"
          ...
          {activeAnalysts.includes('zhipu') && zhipuModelName && (
            <AnalystPanel
              title="Zhipu AI"
          ...
          {activeAnalysts.includes('groq') && groqModelName && (
            <AnalystPanel
              title="Groq"
          ...
          {activeAnalysts.includes('groqNew') && groqNewModelName && (
            <AnalystPanel
              title="Groq (Alt)"
          ...
          {activeAnalysts.includes('groqAlt2') && groqAlt2ModelName && (
            <AnalystPanel
              title="Groq (Alt 2)"
          ...
          {activeAnalysts.includes('openrouter') && openrouterModelName && (
            <AnalystPanel
              title="OpenRouter"
