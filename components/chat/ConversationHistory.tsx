            ...
            {(conversations || []).length === 0 ? (
              ...
              <ul className="space-y-3">
                {(conversations || []).map((conv) => (
                  <li 
                    key={conv.id} 
                    className={`group relative rounded-xl border transition-all duration-200 ${activeConversationId === conv.id ? 'bg-cyan-900/20 border-cyan-500/50 shadow-[0_0_15px_-5px_rgba(212,212,216,0.2)]' : selectedIds.includes(conv.id) ? 'bg-blue-900/20 border-blue-500/50' : 'bg-zinc-800/30 border-white/5 hover:bg-zinc-800/60 hover:border-white/10'}`}
                  >
